...
out_csv = Path("outputs/tables/manga_dap_channels_full.csv")
...
with fits.open(dapall_fits, memmap=True) as hdul:
    ...
    if spi_name is None:
        raise RuntimeError(f"Could not resolve D4000/Dn4000 channel from SPI map")
    if oiii_name is None:
        raise RuntimeError(f"Could not resolve OIII channel from ELG map")
    if ha_name is None:
        raise RuntimeError(f"Could not resolve Halpha channel from ELG map")
